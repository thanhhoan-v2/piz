...
import { Bell, RefreshCw, Send, User, WifiOff } from "lucide-react"
...
export const ChatBox: FC<ChatBoxProps> = ({ roomId, userId, userName }): ReactElement => {
	...
	const checkForUserMention = useCallback(
		(message: ChatMessage) => {
				...
				.on("broadcast", { event: "chat_message" }, (payload: { payload?: ChatMessage }) => {
					...
					if (payload.payload && payload.payload.userId !== userId) {
						...
						if (isUsernameMentioned || isUserIdMentioned) {
							...
							try {
								createChatMentionNotification(message.userId, userId, roomId, message.message)
	...
	const updateMessageInDatabase = async (messageText: string) => {
		...
		try {
			...
			if (userMentions.length > 0) {
				...
				for (const mentionedUser of userMentions) {
					...
					if (mentionedUser.id !== userId) {
						try {
							await createChatMentionNotification(userId, mentionedUser.id, roomId, messageText)
			...
			const { error } = await supabase.from("CollabChat").insert(newChatMessage)
	...
	const handleSelectUser = (id: string, userName: string) => {
		...
		const afterMention =
			afterCurrentWord > -1 ? newMessage.substring(startMentionIndex + afterCurrentWord + 1) : ""
			...
			while (true) {
				...
				parts.push(
					<Badge
						...
						variant="secondary"
						className={`mx-1 px-2 py-0.5 inline-flex items-center ${
							isCurrentUser
			...
			<div className="p-3 bg-gray-800 border-b border-gray-700 flex justify-between items-center">
				<div className="flex items-center gap-2">
					...
					{hasUnreadMention && (
						<Badge
							variant="secondary"
							className="bg-yellow-600/40 text-yellow-200 animate-pulse px-2 py-1"
						>
							...
							>
								<div className="text-xs opacity-80 mb-1 flex items-center gap-1">
									...
									{checkForUserMention(msg) && msg.userId !== userId && (
										<Badge
											variant="secondary"
											className="bg-yellow-600/30 text-yellow-200 text-[9px] px-1"
										>
											Mentioned you
										</Badge>
									)}
								</div>
								<div className="break-words">{formatMessageWithMentions(msg.message)}</div>
							</div>
						</div>
					))
				)}
					...
					{showMentionSuggestions && (
						<div className="bg-gray-700 rounded-t-lg mb-1 max-h-[150px] overflow-y-auto shadow-lg border border-gray-600">
							{isSearching ? (
								...
								<div className="px-3 py-2 text-gray-300 text-center">No users found</div>
							)}
						</div>
					)}
