...
export async function createTeamJoinRequest({
	...
	try {
		...
		// For now, we'll consider all team members as having permission
		// In a real implementation, we would check each member's permissions
		const teamMembersWithPermission = teamUsers
		...
		await Promise.all(
			teamMembersWithPermission.map(async (teamMember) => {
				try {
					...
					console.error(
						`[TEAM_JOIN_REQUEST] Error creating notification for ${teamMember.id}:`,
						error
					)
				}
			})
